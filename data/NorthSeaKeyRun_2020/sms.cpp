...
model_data::model_data(int argc,char * argv[]) : ad_comm(argc,argv)
{
 ...
 if (npr==0) npr=1;
  ...
  spawning_q.allocate(first_VPA,nsp);
 ...
 check_yrpred(OP_consum)
  OP_n_proportion_M2.allocate(1,2,first_VPA,pnsp);
 ...
 if (read_HCR_file==1) if (test_output==51) cout<<"hcr.input:"<<endl<<tmp2<<endl;
  ...
  F_cap_true.allocate(first_VPA,pnsp);
  F_cap_forecast.allocate(first_VPA,pnsp);
 ...
 if (read_cov==0) { ad_comm::change_datafile_name("just_one.in"); tmpA=-100;}
  ...
  coBlock.allocate("coBlock");
  coDim.allocate(1,coBlock,"coDim");
  coDimn.allocate(1,coBlock);
 coDimn=coDim;
  coDimn1.allocate(1,coBlock,1,coDimn);
 coDimn1=1;
  coDimn2.allocate(1,coBlock,1,coDimn);
 for (k=1;k<=coBlock;k++) for(i=1;i<=coDim(k);i++) coDimn2(k,i)=coDim(k); 
  coVariance_n.allocate(1,coBlock,1,coDimn,coDimn1,coDimn2,"coVariance_n");
 ...
 if (read_cov==0) { ad_comm::change_datafile_name("just_one.in"); tmpA=-100;}
  ...
  cofBlock.allocate("cofBlock");
  cofDim.allocate(1,cofBlock,"cofDim");
  cofDimn.allocate(1,cofBlock);
 cofDimn=cofDim;
  cofDimn1.allocate(1,cofBlock,1,cofDimn);
 cofDimn1=1;
  cofDimn2.allocate(1,cofBlock,1,cofDimn);
 for (k=1;k<=cofBlock;k++) for(i=1;i<=cofDim(k);i++) cofDimn2(k,i)=cofDim(k);
  coVariance_nf.allocate(1,cofBlock,1,cofDimn,cofDimn1,cofDimn2,"coVariance_nf");
 if (test_output==53 && read_HCR_file==1 && read_cov==1) 
 ...
 if (read_cov==0) { ad_comm::change_datafile_name("just_one.in"); tmpA=-100;}
  ...
  decBlock.allocate("decBlock");
  decDim.allocate(1,decBlock,"decDim");
  decDimn.allocate(1,decBlock);
 decDimn=decDim;
  decDimn1.allocate(1,decBlock,1,decDimn);
 decDimn1=1;
  decDimn2.allocate(1,decBlock,1,decDimn);
 for (k=1;k<=decBlock;k++) for(i=1;i<=decDim(k);i++) decDimn2(k,i)=decDim(k);
  decomposed_n.allocate(1,decBlock,1,decDimn,decDimn1,decDimn2,"decomposed_n");
 if (test_output==53 && read_HCR_file==1 && read_cov==1) 
 ...
 if (read_cov==0) { ad_comm::change_datafile_name("just_one.in"); tmpA=-100;}
  ...
  decfBlock.allocate("decfBlock");
  decfDim.allocate(1,decfBlock,"decfDim");
  decfDimn.allocate(1,decfBlock);
 decfDimn=decfDim;
  decfDimn1.allocate(1,decfBlock,1,decfDimn);
 decfDimn1=1;
  decfDimn2.allocate(1,decfBlock,1,decfDimn);
 for (k=1;k<=decfBlock;k++) for(i=1;i<=decfDim(k);i++) decfDimn2(k,i)=decfDim(k);
  decomposed_nf.allocate(1,decfBlock,1,decfDimn,decfDimn1,decfDimn2,"decomposed_nf");
 if (test_output==53 && read_HCR_file==1 && read_cov==1) 
...
v_last_Osp=nOthPred;
 ...
 if (select_species==1)  change_input("selected_species.in") else  change_input("just_one.in");
  selected_sp_like.allocate(first_VPA,nsp,"selected_sp_like");
 ...
 if (multi>0) ad_comm::change_datafile_name("n_proportion_m2.in");   
  ...
  N_prop_M2_input.allocate(first_VPA,nsp,fyData,lyData,fq,lq,fa,max_a,"N_prop_M2_input");
 if (test_output==2) cout<<"Proportion of N used for calculation of M2 from  file n_proportion_m2.in:"<<setfixed()<<setprecision(3)<<endl<<N_prop_M2_input<<endl;
  check10a.allocate("check10a");
  if (multi>0) checkSum(check10a,"n_proportion_m2.in");
 y=lyData; q=lq; CALC_yq
  N_prop_M2.allocate(1,yq,first_VPA,v_last_sp,vVPA_fa,vVPA_la);
 ...
 if (test_output==2) cout<< "known_recruitment:"<<endl<<known_recruitment<<endl;
  use_known_rec_option_by_sp.allocate(first_VPA,nsp);
 ...
 if (read_HCR_file==1)if (assess_uncertanty(1,1)>=2 && assess_uncertanty(1,1)<=5) rep=assess_uncertanty(1,4);
  N_obs_rep.allocate(first_VPA,nsp,1,rep,fa,max_a);
  F_obs_rep.allocate(first_VPA,nsp,1,rep,fq,lq,fa,max_a);
 ...
 sdReportYear=lyModel-fyModel;        // number of years in SD report for mean F and SSB
  ...
  cons_multiplier_options.allocate(1,Mnpr,1,3,"cons_multiplier_options");
  lb_cons_multiplier_options.allocate(1,Mnpr);
  ub_cons_multiplier_options.allocate(1,Mnpr);
  ph_cons_multiplier_options.allocate(1,Mnpr);
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
 ...
 xx=min_catch_CV*min_catch_CV;       
  catch_s2_ini.allocate(first_VPA,i,1,seasonal_combined_catch_s2,1,n_catch_s2_group,xx,2.0,phase_catch_s2_ini_vec,"catch_s2_ini");
 ...
 stl_prey_avail_part=0;                
  ...
  #endif
  cons_multiplier.allocate(1,cons_multiplier_Mnpr,lb_cons_multiplier_options,ub_cons_multiplier_options,ph_cons_multiplier_options,"cons_multiplier");
 ...
 for (s=first_VPA;s<=nsp;s++) for (a=fa;a<=la_VPA(s);a++) na_sp++;
  GMminOne.allocate(1,nsp,"GMminOne");
  #ifndef NO_AD_INITIALIZE
    GMminOne.initialize();
  #endif
 ...
 for (s=first_VPA;s<=nsp;s++) for (q=1;q<=lq;q++) for (a=faq(q);a<=las(s);a++) if (a >=cfa(s)) na_spFQ++ ;
  log_exploi_pattern.allocate(1,na_spFQ,"log_exploi_pattern");
  #ifndef NO_AD_INITIALIZE
    log_exploi_pattern.initialize();
  #endif
 ...
 for (s=first_VPA;s<=nsp;s++) if (is_prey(s)==1) for (y=lyModel-sdReportYear;y<=lyModel;y++) {parNo++;  parexp<<"M2_sd0 "<<parNo<<" "<<s<<" "<<y<<" -1 -1 0 -1 -1 -1 "<<s<<" "<<y<<" -1"<<endl; }
  M2_sd1.allocate(1,nprey,lyModel-sdReportYear,lyModel,"M2_sd1");
...
void model_parameters::calc_exploitation_pattern(void)
{
 int s,y,a,q,rc,i;
 int yq;
...
void model_parameters::do_assessment(int s,int rep, dmatrix N, dmatrix F)
{
...
dmatrix model_parameters::exploitation_fromF(int s,dmatrix F)
{
 int q,a;
 double sum;
 sum=0;
...
double model_parameters::find_Fscaling_from_target_SSB_rep(int s,int rep,int first_q,double lower,double upper,double target_SSB,dmatrix M,dmatrix M1,dmatrix M2,dmatrix pred_F,dmatrix N_obs_rep, d3_array F_obs_rep, dmatrix west, dmatrix propmat,double rec1, int year)
{
  double dif,x,y;
  double SSB0;
...
dvector model_parameters::age_uncertanty(int s, dvector un, dvector N, dvector CV)
{
...
void model_parameters::cov_uncertanty(int n,int s,int lastNindex, dvector mu , dmatrix cov,double typeVar )
{
 int k=cov.colmax();
