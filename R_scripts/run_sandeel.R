...
df.tmb <- get_TMB_parameters(
  ...
  recmodel = 1, # Hockey stick recruitment model
  ...
  surveySD =  list(c(0,1),c(1,2)), # Survey age groupings for SD estimates
  catchSD = list(c(1,3), c(1,3)), # Catch age groupings for SD estimates
  estSD = c(0,2,0), # Flags for how to handle SDs for 1) survey, 2) catch, 3) Stock recruitment relationship
  beta = 105809, # Assumed hockey stick break point (SSB),
  nllfactor = c(1,1,0.05) # Manual weighting of 3 log-likelihood components
