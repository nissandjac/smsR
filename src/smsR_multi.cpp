...
// // // // Age
  ...
  DATA_VECTOR(nsurvey); // Number of surveys
  DATA_VECTOR(recseason); // Season where recruitment occurs
  DATA_VECTOR(Fminage);
  DATA_VECTOR(Fmaxage); // Max age to fish
  DATA_VECTOR(useEffort); // use effort or estimate F
  DATA_VECTOR(useBlocks); // Use blocks for species selectivity?
  DATA_VECTOR(estimateCreep); // Bolean, estimate creep from effort data
  DATA_VECTOR(nage); // max age of all species
  DATA_IARRAY(CminageSeason); // Minimum age with fishing mortality per season
  DATA_IARRAY(Qminage); // Minium ages in surveys
  DATA_IARRAY(Qmaxage); // Maximum age in survey
  DATA_IARRAY(Qlastage);
  DATA_IARRAY(bidx); // Indexes for blocks of fishing mortality


  DATA_INTEGER(maxnage); // Number of age groups
  ...
  DATA_ARRAY(isFseason);
  DATA_ARRAY(surveyEnd);
  DATA_ARRAY(surveyStart);
  DATA_ARRAY(surveySeason);

  DATA_VECTOR(minSDsurvey);
  DATA_VECTOR(minSDcatch); // minium catch CV
...
//
// // // Recruitment
  DATA_IVECTOR(recmodel); // Which recruitment model?
  ...
  DATA_IVECTOR(estCV); // Determine which SDs are calculated or estimated
   // Parameters
  PARAMETER_ARRAY(logRin); // Recruitment
  PARAMETER_ARRAY(logNinit); // Initial distribution
  PARAMETER_ARRAY(logFyear);
  PARAMETER_ARRAY(Fseason);
  PARAMETER_ARRAY(logFage);
  PARAMETER_ARRAY(SDsurvey); // Survey variation
  PARAMETER_ARRAY(SDcatch); // Catch SD's
  PARAMETER_VECTOR(creep); // Estimated technical creep
// //
  PARAMETER_ARRAY(logQ);
  PARAMETER_VECTOR(pin);
  PARAMETER_VECTOR(logalpha);
  PARAMETER_VECTOR(logbeta);
  PARAMETER_VECTOR(logSDrec);
  PARAMETER_VECTOR(logSDF); // Fishing mortality variability

//
array<Type>Catch(nage,nyears, nseason, nspecies);
array<Type>CatchN(nage,nyears, nseason, nspecies);
array<Type>Zsave(nage,nyears,nseason, nspecies);
array<Type>Nsave(nage,nyears+1,nseason, nspecies);
array<Type>survey(nage, nyears,nsurvey, nspecies);
array<Type>Qsurv(nage, nsurvey, nspecies);
array<Type>logQsurv(nage, nsurvey, nspecies);
array<Type>SDS(nage, nsurvey, nspecies);
array<Type>SDC(nage, nseason, nspecies);
array<Type>F0(nage, nyears, nseason, nspecies);
array<Type>log_exp_pattern(nage, nseason, nspecies);
array<Type>Fquarter(nage,nyears,nseason, nspecies);
array<Type>Fsel(nage,nyears,nseason, nspecies);
array<Type>p(nage,nsurvey, nspecies);
array<Type>Fagein(nage, nyears, nspecies);
//array<Type>SDR_catch(nage, nyears);


array<Type>SSB(nyears+1, nspecies);
array<Type>Fyear(nyears, nspecies);
array<Type>Rsave(nyears, nspecies);
array<Type>logRec(nyears, nspecies);
array<Type>term_logN_next(nage, nspecies); // Numbers at age in the future year

array<Type>Rin(nyears, nspecies);
array<Type>Catchtot(nyears, nspecies);
// vector<Type>powerin(nsurvey);
array<Type>Q(logQ.ncol(), nspecies);


// Assign vectors
vector<Type>alpha(nspecies);
vector<Type>beta(nspecies);
vector<Type>SDrec(nspecies);
vector<Type>SDF(nspecies);
...
for(int species=0;species<nspecies;species++){
  ...
  alpha(species) = exp(logalpha(species));
  beta(species) = exp(logbeta(species));
  SDrec(species) = exp(logSDrec(species));
  SDF(species) = exp(logSDF(species));
  Fyear(0,species) = 1;
...
for(int species=0;species<nspecies;species++){
  for(int time=0;time<nyears;time++){
        Rin(time, species)=exp(logRin(time,species));
        if(time > 0){
          Fyear(time,species) = exp(logFyear(time-1,species));
        }
  }
}

for(int species=0;time<nspecies;i++){
  for(int i=0;i<(logQ.size());i++){ //
    Q(i,species) = exp(logQ(i), species);
  }
}
...
for(int species=0;time<nspecies;i++){
  ...
  for(int i=0;i<(nage(species));i++){ //
    for(int j=0;j<(nyears);j++){

    if(i >= Fminage(species)){

      if(i < Fmaxage(species)){
        Fagein(i,j,species) = exp(logFage(i-Fminage, bidx(j),species));
      }else{
        Fagein(i,j,species) = exp(logFage(Fmaxage-Fminage,bidx(j),species));
        }
      }
     }
  }
...
if(useBlocks == 0){
  for(int species=0;time<nspecies;i++){
      for(int j=0;j<(nyears);j++){
        for(int i=0;i<nage(species);i++){ // Loop over other ages (recruits excluded)
          for(int qrts=0;qrts<nseason;qrts++){ // Loop over other ages

            if(isFseason(qrts, species) == 1){
              if(i < CminageSeason(qrts, species)){
              Fquarter(i,j,qrts, species) = Type(0.0);

            }else{

              if(useEffort(species) == 0 && nseason > 1){

                if(i >= CminageSeason(qrts,species)){ //  && i <= Fmaxage Is not used for some reason
                Fquarter(i,j,qrts, species) = Fseason(qrts,i-(CminageSeason(qrts, species)), species);
                }
                // if(i > Fmaxage){
                // Fquarter(i,j,qrts) = Fseason(qrts, Fmaxage);
                // }
            }else{
              Fquarter(i,j,qrts) = Fseason(qrts,bidx(j), species);
            }
              ...
              if(i < CminageSeason(qrts, species)){
              Fquarter(i,j,qrts, species) = 0;//Type(1);
              }else{
              if(i == 0){
                if(nseason == 2){
                    Fquarter(i,j,qrts, species) = Type(1);
                  }else{
                    Fquarter(i,j,qrts, species) = Type(1)/nseason; // This is a hack - ask Morten
                }
                }else{
                Fquarter(i,j,qrts, species) = Type(1)/nseason; // Where does this come from?
                }
              }
            }
          }
        ...
        for(int i=0;i<nage(species);i++){ // Loop over other ages (recruits excluded)
          for(int qrts=0;qrts<nseason;qrts++){ // Loop over other ages
            ...
            if(isFseason(qrts,species) == 1){
              ...
              Fquarter(i,j,qrts,species) = Fseason(qrts,bidx(j),species);
              }
            }else{
...
// Estimate creep
if(useEffort(species) == 1){
  if(estimateCreep(species) == 1){
    for(int species=0;time<nspecies;i++){
          for(int time=0;time<(nyears);time++){ // Loop over years excluding last one
              for(int qrts=0;qrts<nseason;qrts++){ // Loop over other seasons
                ...
                if(time == 0){
                  effort_new(0,qrts,species) = 1*effort(time,qrts,species);
                }else{
                effort_creep(time,species) = effort_creep(time-1,species)*exp(creep,species);
                effort_new(time,qrts,species) = effort(time, qrts,species)*effort_creep(time,species);
              }
            }
          }
  }else{
    ...
    if(useEffort(species) == 1){
      for(int species=0;time<nspecies;i++){
        for(int time=0;time<(nyears);time++){ // Loop over years excluding last one
          for(int qrts=0;qrts<nseason;qrts++){ // Loop over other seasons
            effort_new(time,qrts,species) = effort(time, qrts,species);
        }
      }
    }
  }
    ...
    for(int time=0;time<(nyears);time++){ // Loop over years excluding last one
      for(int i=0;i<nage(species);i++){ // Loop over other ages (recruits excluded)
        for(int qrts=0;qrts<nseason;qrts++){ // Loop over other seasons
          ...
          //  if(qrts != (nseason-1)){
              if(nocatch(time, qrts,species)>0){
              F0(i,time,qrts,species) = Fquarter(i,time,qrts,species)*Fagein(i,time,species)*effort_new(time, qrts,species);
              Fsel(i,time,qrts,species,species) = log(Fquarter(i,time,qrts,species)*Fagein(i,time,species));
              }


              if(time == (nyears-1)){
                  if(Fquarter(i, time, qrts,species) > 0 && Fagein(i,time,species)> 0){

                  log_exp_pattern(i,qrts,species) = log(Fquarter(i,time,qrts,species)*Fagein(i,time),species);
                }else{
                  log_exp_pattern(i, qrts,species) = -1000;
                }
              }

             Zsave(i,time,qrts,species) = F0(i,time,qrts,species)+M(i,time,qrts,species);
           }
      }
    }
  }
    ...
    for(int time=0;time<(nyears);time++){ // Loop over years excluding last one
      for(int i=0;i<nage;i++){ // Loop over other ages (recruits excluded)
        for(int qrts=0;qrts<nseason;qrts++){ // Loop over other seasons
          ...
          if(nocatch(time, qrts,species)>0){
            F0(i,time,qrts,species) = Fquarter(i,time,qrts,species)*Fagein(i,time,species)*Fyear(time,species);
            Fsel(i,time,qrts,species) = log(Fquarter(i,time,qrts,species)*Fagein(i,time,species));
          }
