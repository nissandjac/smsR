...
SR.b <- SR %>% dplyr::select(-SE) %>% dplyr::mutate(fleet = paste('survey:',survey)) %>% dplyr::rename(residual = ResidSurvey) %>% dplyr::select(-survey)
CR.b <- CR %>% dplyr::mutate(fleet = paste('catch season:', season)) %>% dplyr::rename(residual = ResidCatch) %>% dplyr::select(-season)
...
p <- ggplot(dat.plot, ggplot2::aes(x = years, y = as.character(ages), color = factor(col)))+
  ...
  facet_wrap(~fleet)+
  theme_classic() +ggplot2::scale_size(range = ss)+
  ggplot2::scale_color_manual(values = c('red','blue'))+scale_x_continuous('Year')+
  ggplot2::scale_y_discrete('Age')+theme(legend.title = element_blank(), legend.position = 'top')+
