...
DATA_SECTION
 ...
 // Normalise to mean effort=1   SKAL �NDRES TIL NORMALISERES INDENFOR �RR�KKE MED SAMME EXPLOITATION PATTERN
 number sum_effort
 !! if (any_do_effort==1) {
...
//*********************************************************************************************
 ...
 3darray index_maxl(1,def_yqd,1,yqdAll,yqdsAll_fa,yqdsAll_la) // last size class in use
  ...
  4darray size_l_sea(1,def_yqd,1,nsp,fa,max_a,minl,maxl)    //length or weight of species at age in the sea BURDE V�RE "RAGGED"
...
FUNCTION void calc_M2_ALK(int y, int q, int d, dvector other_food, d3_array size,d3_array consum, dvar_matrix N_bar, dvar_matrix N, dvar_matrix F, dvar_matrix M1, dvar_matrix& M2);
 ...
 for (prey=first_VPA;prey<=nsp;prey++) if (is_prey(prey)==1) {
   for (prey_a=faq(q);prey_a<=la(prey);prey_a++) {
     ...
     for (prey_l=int(index_minl(yqd,prey,prey_a));prey_l<=int(index_maxl(yqd,prey,prey_a));prey_l++) {
       if (use_Nbar==0) {                    // VR�VL ???? Nej OK  Use N in the mean N to calc dead numbers
   ...
   for (s2_group=1;s2_group<=n_catch_s2_group(s);s2_group++) {
     ...
     for (y=fyModel;y<=lyModel;y++) {
        if (seasonal_annual_catches(s)==0) {
         ...
         else {     // use annual catches in objective function, k�nt er det ikke
...
FUNCTION  evaluate_SSB_recruitment_contributions
 ...
 double mins,epsilon;
  ...
  gamma=10; // constant from Benoit Mesnil and Marie-Jo�lle Rochet, A continious hocky stick stock-recruit model for estimating MSY reference points. ICES Journal of Marine Science, 67: June 8, 2010
