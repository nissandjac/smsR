...
getDataSMS_multi <- function(wd, spp = NULL, is.dynamic = 1,
  ...
  survey.years <- survey.start <- survey.age <- survey.catch <- list()
  power.model <- survey.season <- survey.nvar <- rep(NA, length(spp))
  survey.names <- rep(spp[which(is.dynamic == 1)], each = nsurveys)
  # survey.quarter = NA,
  # 
