...
getDataSandeel <- function(wd,
  ...
  mtrx <- list(M = df_to_matrix(M, quarter = seasons),
               mat = df_to_matrix(mat, quarter = seasons),
               west = df_to_matrix(west, quarter = seasons),
               weca = df_to_matrix(weca, quarter= seasons))
