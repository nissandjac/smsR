...
# Weight at age
  ...
  p1 <- ggplot(waa, aes(x = years, y = value, color = model, group = season))+geom_line()+facet_wrap(~age)+theme_classic()
  ...
  p2 <- ggplot(waa, aes(x = years, y = value, color = model))+geom_line()+facet_grid(season~age)+theme_classic()
  ...
  p3 <- ggplot(eff, aes(x = years, y = effort, color = model))+geom_line()+facet_wrap(~season)+theme_classic()
