...
# Test the different harvest control rules #

# Bescape
x1 <- getForecastTable(df.tmb, sas, HCR = 'Bescape', Btarget = 100000*exp(0.2))
x2 <- getTAC(df.tmb, sas, Btarget = 100000*exp(0.2))


getForecastTable(df.tmb, sas, HCR = 'Fmsy', Flimit = 0.3)
getTAC(df.tmb, sas, HCR = 'Fmsy', Flimit = 0.3)


# Now calculate Fmsy

Fmsy <- getFmsy(sas, plotMSY = TRUE)
